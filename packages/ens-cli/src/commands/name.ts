 ...
 * Name Command
 *
 * Assign an ENS subname to a smart contract or address.
 * Follows ENS standards for subname creation.
 */
...
/**
 * Name a smart contract with an ENS subname
 */
export async function nameContract(options: NameContractOptions) {
  ...
  try {
    ...
    console.log(`${colors.blue("Network:")}      ${options.network || "sepolia"}`);
